...
/datum/hud/human/FinalizeInstantiation(var/ui_style='icons/mob/screen1_White.dmi', var/ui_color = "#ffffff", var/ui_alpha = 255)
	...
	mymob.use_zoom = new /obj/screen()
	mymob.use_zoom.icon = ui_style
	mymob.use_zoom.icon_state = "fixeye"
	mymob.use_zoom.name = "look into distance"
	mymob.use_zoom.screen_loc = ui_zoom
	hud_elements += mymob.use_zoom
	...
	if(hud_data.has_hydratation)
		...
		mymob.hydratation_icon.icon_state = "hydratation0"
		mymob.hydratation_icon.name = "hydratation"
		mymob.hydratation_icon.screen_loc = ui_nutrition
		hud_elements |= mymob.hydratation_icon
