...
/turf/simulated/wall/bullet_act(var/obj/item/projectile/Proj)
	var/p_x = Proj.p_x + pick(0,0,0,0,0,-1,1)
	var/p_y = Proj.p_y + pick(0,0,0,0,0,-1,1)
	var/decaltype = 1
	playsound(src, "hitobject", 75, 1)
	if(ricochet_id != 0)
	...
	var/proj_damage = Proj.get_structure_damage() / 6
	...
	if(decaltype == 1)
		...
		if(Proj.damage >=20 || istype(Proj, /obj/item/projectile/beam/practice))
			...
			BM.set_dir(pick(NORTH,SOUTH,EAST,WEST))
...
/turf/simulated/proc/clear_bulletholes()
