...
/obj/item/device/communicator/proc/open_connection_to_ghost(mob/user, var/mob/candidate)
	...
	sleep(10)
	to_chat(new_voice, "<span class='notice'>\icon[src] Dialing. . .</span>")
	sleep(20)
...
/obj/item/device/communicator/see_emote(mob/living/M, text)
	...
	for(var/obj/item/device/communicator/comm in communicating)
		...
		if(!T) return
		var/list/mobs_to_relay = list() //Range of 3 since it's a tiny video display
		var/list/listen_turfs = list()
		get_mobs_and_objs_in_view_fast(T,world.view,mobs_to_relay, listen_turfs)
...
/mob/observer/dead/verb/join_as_voice()
	...
	for(var/mob/living/L in SSmobs.mob_list) //Simple check so you don't have dead people calling.
