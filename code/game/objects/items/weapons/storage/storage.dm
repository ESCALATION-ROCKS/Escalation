...
/obj/item/weapon/storage/proc/close(mob/user as mob)
	hide_from(user)
	playsound(src.loc, close_sound, 50, 1)
...
/obj/item/weapon/storage/backpack/attack_hand(mob/user as mob)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		if(H.back == src && !H.get_active_hand() && !src.reachable)
			to_chat(user, "<span class='warning'>You cannot reach the [src] when it on your back.</span>")
			return
	if (src.loc == user)
		src.open(user)
	else
