...
/obj/item/weapon/card/id/syndicate/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	if (!ui)
		ui = new(user, src, ui_key, "agent_id_card.tmpl", "Agent passport", 600, 400)
...
/obj/item/weapon/card/id/refugeecard/replicant
	name = "refugee card"
...
/obj/item/weapon/card/id/refugeecard/replicant/attack_self(mob/user as mob)
	// We use the fact that registered_name is not unset should the owner be vaporized, to ensure the id doesn't magically become unlocked.
	if(!registered_user && register_user(user))
		to_chat(user, "<span class='notice'>The microscanner marks you as its owner, preventing others from accessing its internals.</span>")
	else
...
/obj/item/weapon/card/id/refugeecard/replicant/proc/register_user(var/mob/user)
	if(!istype(user) || user == registered_user)
		return FALSE
	registered_user = user
	user.set_id_info(src)
...
/obj/item/weapon/card/id/refugeecard/cheapreplicant/
	var/used = 0

/obj/item/weapon/card/id/refugeecard/cheapreplicant/attack_self()
	if(used == 0)
		..()
		age = rand(19,25)
