/obj/item/weapon/grenade/flashbang
	...
	proc/bang(var/turf/T , var/mob/living/carbon/M)					// Added a new proc called 'bang' that takes a location and a person to be banged.
		to_chat(M, "<span class='extrafuck'>BANG</span>")// Called during the loop that bangs people in lockers/containers and when banging
		playsound(src.loc, 'sound/weapons/flashbang.ogg', 80, 1, 30)		// people in normal view.  Could theroetically be called during other explosions.
																	// -- Polymorph
...
//Now applying sound
		if((get_dist(M, T) <= 2 || src.loc == M.loc || src.loc == M))
			if(ear_safety > 0)
				M.confused += 2
				M.Weaken(1)
				M.playsound_local(src, sound('sound/weapons/flash_ring.ogg', repeat = 0, wait = 0, volume = 60, channel = 2))
			else
				M.playsound_local(src, sound('sound/weapons/contusion.ogg', repeat = 0, wait = 0, volume = 100, channel = 2))
				M.confused += 10
				M.Weaken(3)
			...
			if(!ear_safety)
				M.playsound_local(src, sound('sound/weapons/contusion.ogg', repeat = 0, wait = 0, volume = 70, channel = 2))
				M.confused += 8
				M.ear_damage += rand(0, 6)
				M.ear_deaf = max(M.ear_deaf,10)

		else if(!ear_safety)
			M.playsound_local(src, sound('sound/weapons/flash_ring.ogg', repeat = 0, wait = 0, volume = 100, channel = 2))
			M.confused += 4
