...
/atom/proc/do_climb(var/mob/living/user)
	...
	climbers |= user
	if(user.dex > 8)
		user.visible_message("<span class='warning'>\The [user] leaps onto \the [src]!</span>")
		climbers -= user
		user.forceMove(get_turf(src))
		return 0

	if(!do_after(user,(issmall(user) ? 30 : 30) / user.sstatmodifier(user.dex), src))
		climbers -= user
		return
...
/client/MouseEntered(var/atom/a)
	if(mob && ishuman(mob))
		...
		if(a.mouse_opacity)  // i spread this out to make it more "readable"
			H.hovertext.maptext = null /*"<center><span style=\"\
			color: #188A01; \
			font-family: 'Arial Black', Gadget, sans-serif; \
			\">[uppertext(a.name)]\
			</span></center>"*/
		else
			H.hovertext.maptext = null  // ui is blank, sad!
