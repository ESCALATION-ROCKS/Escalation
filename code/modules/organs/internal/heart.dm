...
/obj/item/organ/internal/heart/proc/handle_blood()
	...
	if(!owner || owner.stat == DEAD || owner.bodytemperature < 170)
		return
	...
	if(pulse != PULSE_NONE || robotic >= ORGAN_ROBOT)
		...
		for(var/obj/item/organ/external/temp in owner.organs)
			...
			if(temp.status & ORGAN_ARTERY_CUT && !temp.tourniqueted)
				var/bleed_amount = Floor((owner.vessel.total_volume / (temp.applied_pressure ? 600 : 250))*temp.arterial_bleed_severity)
				if(bleed_amount)
					if(open_wound || !temp.applied_pressure)
		...
		if(CE_HEMOSTATIC in owner.chem_effects) // aminocaproic
			blood_max *= 0.5

		if(/datum/modifier/mutation/bethaemopoesis in owner.modifiers)
			blood_max *= 0.4

		if(/datum/modifier/mutation/haemophilia in owner.modifiers)
			blood_max *= 3.0
