...
/obj/item/organ/external/New(var/mob/living/carbon/human/holder)
	..()
	if(mThickbones in holder.mutations)
		min_broken_damage *= 2.5
	if(mAvianbone in holder.mutations)
		min_broken_damage *= 0.4
...
/obj/item/organ/external/Destroy()
	...
	if(tourniqueted && tourniqueted.loc == src)
		qdel(tourniqueted)
	tourniqueted = null
...
/obj/item/organ/external/proc/createwound(var/type = CUT, var/damage, var/surgical)
	...
	if(!surgical && (type in list(CUT, PIERCE, BRUISE)) && damage > 25 && local_damage > 40)
		...
		if(internal_damage)
			...
			playsound(src, "serinjury", rand(80,100), 1, 1)
...
INFECTION_LEVEL_THREE	above this germ level the player will take additional toxin damage per second, and will die in minutes without
						antitox. also, above this germ level you will need to overdose on doxicycline to reduce the germ_level.
...
/obj/item/organ/external/proc/droplimb(var/clean, var/disintegrate = DROPLIMB_EDGE, var/ignore_children, var/silent)
	...
	switch(disintegrate)
		if(DROPLIMB_EDGE)
			playsound(src, "bodysplat", 100, 1, 0)
			if(!clean)
...
/obj/item/organ/external/proc/_clamp()
	var/rval = 0
...
/obj/item/organ/external/proc/disfigure(var/type = "brute")
	...
	if(owner)
		if(type == "brute")
			playsound(src.loc, 'sound/effects/body/disfigure.wav', 70, 1, -2)
...
/obj/item/organ/external/proc/jointlock(mob/attacker)
	...
	if(armor < 100)
		...
		var/max_halloss = round(owner.species.total_health / owner.mstatmodifier(owner.end) * 0.8 * ((100 - armor) / 100)) //up to 80% of passing out, further reduced by armour
