...
/datum/turbolift/proc/emergency_stop()
	...
	target_floor = null
	open_doors()
	playsound(control_panel_interior.loc, 'sound/effects/elevator/elevator_stop.wav', 100, 1)
...
/datum/turbolift/proc/open_doors(var/datum/turbolift_floor/use_floor = current_floor)
	playsound(control_panel_interior.loc, 'sound/effects/elevator/elevator_stop1.wav', 100, 1)
	for(var/obj/machinery/door/airlock/door in (use_floor ? (doors + use_floor.doors) : doors))
		door.command("open")
	return

/datum/turbolift/proc/close_doors(var/datum/turbolift_floor/use_floor = current_floor)
	playsound(control_panel_interior.loc, 'sound/effects/elevator/elevator_start1.wav', 100, 1)
	for(var/obj/machinery/door/airlock/door in (use_floor ? (doors + use_floor.doors) : doors))
		door.command("close")
	return
...
/datum/turbolift/proc/do_move()
	...
	if(target_floor == current_floor)
		...
		playsound(control_panel_interior.loc, origin.arrival_sound, 50, 0)
	...
	playsound(control_panel_interior.loc, 'sound/effects/elevator/elevator_move_loop1.wav', 100, 0, extrarange = 2)
