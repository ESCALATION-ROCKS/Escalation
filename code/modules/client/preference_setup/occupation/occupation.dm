...
/datum/category_item/player_setup_item/occupation/content(mob/user, limit = 16, list/splitJobs, splitLimit = 1)
	...
	for(var/datum/job/job in job_master.occupations)
		...
		if(job.minimum_character_age && user.client && (user.client.prefs.age < job.minimum_character_age))
			...
			continue
		if(job.minimum_character_str && user.client && (user.client.prefs.str < job.minimum_character_str))
			. += "<del>[rank]</del></td><td> \[MINIMUM CHARACTER STR: [job.minimum_character_str]]</td></tr>"
			continue
		if(job.minimum_character_end && user.client && (user.client.prefs.end < job.minimum_character_end))
			. += "<del>[rank]</del></td><td> \[MINIMUM CHARACTER END: [job.minimum_character_end]]</td></tr>"
			continue
		if(job.minimum_character_dex && user.client && (user.client.prefs.dex < job.minimum_character_dex))
			. += "<del>[rank]</del></td><td> \[MINIMUM CHARACTER DEX: [job.minimum_character_dex]]</td></tr>"
			continue
		if(job.minimum_character_int  && user.client && (user.client.prefs.int < job.minimum_character_int))
			. += "<del>[rank]</del></td><td> \[MINIMUM CHARACTER INT: [job.minimum_character_int]]</td></tr>"
			continue
		if(job.minimum_character_soc && user.client && (user.client.prefs.soc < job.minimum_character_soc))
			. += "<del>[rank]</del></td><td> \[MINIMUM CHARACTER SOC: [job.minimum_character_soc]]</td></tr>"
			continue
