...
/datum/reagent/tramadol/novomedolum
	name = "Novomedolum"
	description = "An effective and very addictive painkiller. Don't mix with alcohol."
	taste_description = "bitterness"
	color = "#800080"
...
/datum/reagent/tramadol/novomedolum/overdose(var/mob/living/carbon/M, var/alien)
	..()
	M.druggy = max(M.druggy, 10)
...
/datum/reagent/nicotine/shitty/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		if(prob(volume*50))
			H.add_chemical_effect(CE_PULSE, 1)
		if(prob(10))
			to_chat(H,"<span class='warning'>That feels disgusting.</span>")
		if(prob(5))
			to_chat(H,"<span class='warning'>Your lungs sounds badly.</span>")
		if(prob(3))
...
/datum/reagent/talotropine/affect_blood(var/mob/living/carbon/human/M, var/alien, var/removed)
	if(alien == IS_DIONA)
		return
...
/datum/reagent/tetracetam/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	if(alien == IS_DIONA)
		return
	if(M.chem_doses[type] < 2)	//not that effective after initial rush
		M.add_chemical_effect(CE_PULSE, -0.5)
		M.add_chemical_effect(CE_PAINKILLER, 10)
		M.add_chemical_effect(CE_BRAIN_REGEN, 0.4)
	else
		M.add_chemical_effect(CE_OXYGENATED, 0.5)
		M.add_chemical_effect(CE_BRAIN_REGEN, 1)
	if(ishuman(M))
...
/datum/reagent/oxaprofen/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	M.add_chemical_effect(CE_PAINKILLER, 25)
	M.add_chemical_effect(CE_ANTIVIRAL, 1)

/datum/reagent/oxaprofen/overdose(var/mob/living/carbon/M, var/alien)
	..()
