...
/datum/chatOutput/proc/analyzeClientData(cookie = "")
	...
	if(cookie != "none")
		...
		if (connData && islist(connData) && connData.len > 0 && connData["connData"])
			...
			//Uh oh this fucker has a history of playing on a banned account!! - Disabled alerts because it started acting all fucky after the ckeywhitelist implementation
			if (found.len > 0)
