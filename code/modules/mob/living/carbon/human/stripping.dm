/mob/living/carbon/human/proc/handle_strip(var/slot_to_strip_text,var/mob/living/user,var/obj/item/clothing/holder)
	...
	switch(slot_to_strip_text)
		...
		if("splints")
			...
			return
		if("tourniquet")
			visible_message("<span class='danger'>\The [user] is trying to remove \the [src]'s tourniquet!</span>")
			if(do_after(user, HUMAN_STRIP_DELAY, src, progress = 0))
				remove_tourniquet(user)
			return
		if("sensors")
	...
	if(stripping)
		if(unEquipfrompanel(target_slot))
	...
	else if(user.unEquipfrompanel(held))
...
/mob/living/carbon/human/proc/empty_pockets(var/mob/living/user)
	if(!r_store && !l_store)
		...
		return
	if(r_store)
		unEquipfrompanel(r_store)
	if(l_store)
		unEquipfrompanel(l_store)
...
// Remove all splints.
/mob/living/carbon/human/proc/remove_tourniquet(var/mob/living/user)
	var/removed_tourniquet = 0
	for(var/obj/item/organ/external/o in organs)
		if (o && o.tourniqueted)
			var/obj/item/S = o.tourniqueted
			if(!istype(S) || S.loc != o) //can only remove splints that are actually worn on the organ (deals with hardsuit splints)
				to_chat(user, "<span class='warning'>You cannot remove any tourniquets on [src]'s [o.name] - [o.tourniqueted] is supporting some of the breaks.</span>")
			else
				S.add_fingerprint(user)
	...
	if(removed_tourniquet)
		visible_message("<span class='danger'>\The [user] removes \the [src]'s tourniquetes!</span>")
	else
		to_chat(user, "<span class='warning'>\The [src] has no tourniquetes that can be removed.</span>")
