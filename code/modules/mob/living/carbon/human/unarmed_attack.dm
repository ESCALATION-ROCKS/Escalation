...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/carbon/human/user,var/mob/living/carbon/human/target,var/armour,var/attack_damage,var/zone)
	...
	if(attack_damage >= 5 && armour < 100 && !(target == user) && stun_chance <= attack_damage * 5 * additional_damage) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			if(BP_HEAD, BP_EYES, BP_MOUTH)
				...
				target.apply_effect(attack_damage*4, EYE_BLUR, armour)
...
/datum/unarmed_attack/proc/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	playsound(user.loc, pick(attack_sound), 60, 1, -1)
