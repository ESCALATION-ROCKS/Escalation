...
/mob/living/carbon/human/Stat()
	...
	if(statpanel("Status"))
		...
		var/obj/item/organ/internal/replicell/W = internal_organs_by_name[BP_BIOCELL]
		if(W)
			stat(null, "Bioelectrical cell: [W.stored_biopower]/[W.max_biopower]")
...
/mob/living/carbon/human/show_inv(mob/user as mob)
	...
	dat += "<BR><A href='?src=\ref[src];item=tourniquet'>Remove tourniquet</A>"
...
/mob/living/carbon/human/proc/vomit(var/toxvomit = 0, var/timevomit = 1, var/level = 3)
	set waitfor = 0
	if(!check_has_mouth() || isSynthetic() || !timevomit || !level || (CE_ANTIVOMIT in chem_effects))
		return
	...
	if(!lastpuke)
		...
		if(level > 1)
			...
			if(level > 2)
				...
				Stun(3)
				if(nutrition < 120 && hydratation < 160)
...
/mob/living/carbon/human/proc/set_species(var/new_species, var/default_colour)
	...
	maxHealth = species.total_health * healthmodifier(end)
...
/mob/living/carbon/human/proc/make_adrenaline(amount)
	if(/datum/modifier/mutation/noadrenaline in src.modifiers)
		return 0
	if(stat == CONSCIOUS)
	...
	if(/datum/modifier/mutation/highadrenaline in src.modifiers)
		amount *= 2
...
/mob/proc/mob_rest(var/mob/living/user = usr)
	if(resting && !stunned && !weakened)//The incapacitated proc includes resting for whatever fucking stupid reason I hate SS13 code so fucking much.
		...
		if(do_after(user, 4/user.sstatmodifier(dex)))
			resting = 0
...
/mob/living/carbon/human/proc/get_ins_rank()
	...
	if(istype(w_uniform,/obj/item/clothing/under/))
		var/obj/item/clothing/under/P = w_uniform
		rank = P.insrank
	if(rank_prefix[rank])
