...
/mob/living/carbon/human/updatehealth()
	...
	health = maxHealth - getBrainLoss() - (getBruteLoss() + getFireLoss() + getToxLoss() + getOxyLoss()) // thank you brainmed
...
/mob/living/carbon/human/adjustBruteLoss(var/amount)
	amount = amount*species.brute_mod/healthmodifier(end)
	if(amount > 0)
...
/mob/living/carbon/human/adjustFireLoss(var/amount)
	amount = amount*species.burn_mod/healthmodifier(end)
	if(amount > 0)
...
/mob/living/carbon/human/adjustOxyLoss(var/amount)
	if(!need_breathe())
		return
	var/heal = amount < 0
	amount = abs(amount*species.oxy_mod/healthmodifier(end))
	var/obj/item/organ/internal/lungs/breathe_organ = internal_organs_by_name[species.breathing_organ]
	if(breathe_organ)
		if(heal)
...
/mob/living/carbon/human/apply_damage(var/damage = 0, var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/damage_flags = 0, var/obj/used_weapon = null)
	...
	switch(damagetype)
		if(BRUTE)
			damage = damage*species.brute_mod / healthmodifier(end)
			created_wound = organ.take_damage(damage, 0, damage_flags, used_weapon)
			receive_damage()
		if(BURN)
			damage = damage*species.burn_mod / healthmodifier(end)
