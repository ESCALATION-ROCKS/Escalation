...
/mob/living/carbon/human/bullet_act(var/obj/item/projectile/P, var/def_zone)
	...
	playsound(src.loc, "bul_impact", 60, 0, 1)
...
/mob/living/carbon/human/standard_weapon_hit_effects(obj/item/I, mob/living/user, var/effective_force, var/blocked, var/hit_zone)
	...
	if(user.str)//If they have strength then add it.
		effective_force *= mstatmodifier(user.str)
	...
	if(I.sharp && prob(I.sharpness*4) && statscheck(skill_melee, str, 8, 0, src) && !(affecting.status & ORGAN_ARTERY_CUT))
	...
	if(I.sharp && (I.sharpness*4) && statscheck(skill_melee, dex, 6, 0, src) && !(affecting.status & ORGAN_TENDON_CUT) && affecting.has_tendon)//Yes this is the same exactly probability again. But I'm running it seperate because I don't want the two to be exclusive.
	...
	//Finally if we pass all that, we cut the limb off. This should reduce the number of one hit sword kills.
		...
		if(prob(I.sharpness) && statscheck(skill_melee, str, 16, 0, src))
	...
	if((I.damtype == BRUTE || I.damtype == PAIN) && prob(25 + (effective_force * 2)))
		if(!stat)
			if(headcheck(hit_zone))
				...
				if(prob(effective_force * I.knockout_modifier) && statscheck(skill_melee, dex, 8, 0, src))
					visible_message("<span class='danger'>[src] [species.knockout_message]</span>")
					apply_effect(20 * mstatmodifier(src.end), PARALYZE, blocked)
					playsound(src, "bodyfall", 40, 1, -1)
			else
				//Easier to score a stun but lasts less time
				if(prob(effective_force * I.knockout_modifier) && statscheck(skill_melee, str, 6, 0, src))
					visible_message("<span class='danger'>[src] has been knocked down!</span>")
					apply_effect(6 * mstatmodifier(src.end), WEAKEN, blocked)
					playsound(src, "bodyfall", 40, 1, -1)
...
/mob/living/carbon/human/proc/attack_joint(var/obj/item/organ/external/organ, var/obj/item/W, var/effective_force, var/dislocate_mult, var/blocked)
	...
	var/dislocate_chance = effective_force/(dislocate_mult * organ.min_broken_damage * config.organ_health_multiplier / mstatmodifier(src.end))*15
	if(prob(dislocate_chance * blocked_mult(blocked)))
		visible_message("<span class='danger'>[src]'s [organ.joint] [pick("gives way","caves in","crumbles","collapses")]!</span>")
		organ.dislocate(1)
		playsound(loc, 'sound/effects/body/cavein.wav', 80, 1, -1)
		return 1
	return 0
...
/mob/living/carbon/human/hitby(atom/movable/AM as mob|obj,var/speed = THROWFORCE_SPEED_DIVISOR)
	if(istype(AM,/obj/))
		...
		if (O.throw_source)
			...
			miss_chance = max(15/mstatmodifier(src.dex)*(distance-2), 0)
