...
/mob/living/carbon/human/attack_hand(mob/living/carbon/M as mob)
	...
	if(istype(H))
		...
		if(istype(H.gloves, /obj/item/clothing/gloves/boxing/hologlove))
			...
			var/damage = rand(0, 9)
			if(!damage)
				playsound(loc, 'sound/weapons/punchmiss.ogg', 65, 1, -1)
	...
	switch(M.a_intent)
		...
		if(I_DISARM)
			...
			if(randn <= 80 && statscheck(!skill_melee, dex, 6, 0, src) && M.statscheck(M.skill_melee, M.dex, 4, 0, src))
			...
			playsound(loc, 'sound/weapons/punchmiss.ogg', 40, 1, -1)
