...
/mob/living/carbon/human/equip_to_slot(obj/item/W as obj, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		if(slot_holster)//custom gun slot
			src.holster = W
			W.equipped(src, slot)
			W.screen_loc = ui_holster
			update_inv_wear_holster(redraw_mob)
		if(slot_left_pouch)//custom gun slot
			src.left_pouch = W
			W.equipped(src, slot)
			W.screen_loc = ui_left_pouch
			update_inv_wear_left_pouch(redraw_mob)
		if(slot_right_pouch)//custom gun slot
			src.right_pouch = W
			W.equipped(src, slot)
