...
/mob/living/carbon/human/handle_chemicals_in_body()
	...
	if (hydratation > 0 && stat != 2)
		hydratation = max (0, hydratation - species.thirst_factor)
...
/mob/living/carbon/human/handle_regular_status_updates()
	...
	if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if(get_shock() >= (species.total_health * healthmodifier(end) - 100))
			if(!stat)
...
/mob/living/carbon/human/handle_regular_hud_updates()
	...
	if(stat != DEAD)
		...
		whitenoisealpha = src.getBrainLoss()
		...
		if(nutrition_icon)
			switch(nutrition)
				if(500 to INFINITY)				nutrition_icon.icon_state = "nutrition0"
				if(400 to 500)					nutrition_icon.icon_state = "nutrition1"
				if(300 to 400)					nutrition_icon.icon_state = "nutrition2"
				if(200 to 300)					nutrition_icon.icon_state = "nutrition3"
				if(100 to 200)					nutrition_icon.icon_state = "nutrition4"
				else							nutrition_icon.icon_state = "nutrition5"

		if(hydratation_icon)
			switch(hydratation)
				if(500 to INFINITY)				hydratation_icon.icon_state = "hydratation0"
				if(400 to 500)					hydratation_icon.icon_state = "hydratation1"
				if(300 to 400)					hydratation_icon.icon_state = "hydratation2"
				if(200 to 300)					hydratation_icon.icon_state = "hydratation3"
				if(100 to 200)					hydratation_icon.icon_state = "hydratation4"
				else							hydratation_icon.icon_state = "hydratation5"
