...
//	var/IH = get_item_by_slot(ahand) && !istype(I, IH)
	if(!(force || canUnEquip(I)))
		return
	if(I && !force && I != r_hand && I != l_hand)
		if(do_after(src, I.time_to_unequip))
			if(client)
				client.screen -= I
			I.loc = loc
			I.dropped(src)
...
/mob/proc/unEquipfrompanel(obj/item/I, var/atom/target)
	if(!canUnEquip(I))
		return
	drop_from_inventory(I, target)
	return 1
