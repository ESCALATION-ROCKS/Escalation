...
/datum/grab/normal/on_hit_harm(var/obj/item/grab/G)
	...
	if(!O.dislocated)
		...
		if(do_mob(assailant, affecting, action_cooldown - 1))
			...
			playsound(assailant.loc, 'sound/effects/body/cavein.wav', 80, 1, -1)
			return 1
...
/datum/grab/normal/proc/headbutt(var/obj/item/grab/G)
	...
	if(armor < 50 && target.headcheck(BP_HEAD) && target.statscheck(target.skill_melee, target.end, 8, 0, src))
