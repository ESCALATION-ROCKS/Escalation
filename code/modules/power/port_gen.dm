...
/obj/machinery/power/port_gen/pacman/diesel
	name = "Diesel Portable Generator"
	desc = "A power generator that runs on food. Rated for 100 kW maximum safe output!"
	icon_state = "portgen2"
//	sheet_path = /obj/item/stack/material/biofuel
	sheet_name = null
...
/obj/machinery/power/port_gen/pacman/diesel/examine(mob/user)
	..()
	to_chat(user, "Auxilary tank shows [reagents.total_volume]u of liquid in it.")
...
/obj/machinery/power/port_gen/pacman/diesel/UseFuel()
	if(reagents.has_reagent("fuel"))
		...
		reagents.remove_any(1)
		if(prob(2))
			audible_message("<span class='notice'>[src] churns.</span>")
	else
		temperature_gain = initial(temperature_gain)
	..()

/obj/machinery/power/port_gen/pacman/diesel/attackby(var/obj/item/O, var/mob/user)
	if(istype(O, /obj/item/weapon/reagent_containers/))
		var/obj/item/weapon/reagent_containers/R = O
		if(R.standard_pour_into(src,user))
			if(reagents.has_reagent("fuel"))
				audible_message("<span class='notice'>[src] whirs.</span>")
		return
	..()