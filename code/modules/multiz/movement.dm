...
/mob/living/carbon/human/handle_fall_effect(var/turf/landing)
	if(statscheck(skill_athlethics, dex, 14, 0, src) && !lying)
	...
	..()
	var/damage = 20 / mstatmodifier(dex)
	...
	apply_damage(rand(0, damage), BRUTE, BP_L_HAND)
	apply_damage(rand(0, damage), BRUTE, BP_R_HAND)
	apply_damage(rand(0, damage), BRUTE, BP_GROIN)
	apply_damage(rand(0, damage), BRUTE, BP_R_FOOT)
	apply_damage(rand(0, damage), BRUTE, BP_L_FOOT)
	Stun(1)
	Weaken(1)
	updatehealth()
	playsound(landing, get_sfx("bodyfall"), 100, 1)

/obj/structure/handle_fall_effect(var/atom/hit_atom, var/damage_min = 15, var/damage_max = 30, var/silent = FALSE, var/planetary = FALSE)
